...
ServicifyClient.prototype.resolve = function (name, required) {
  ...
  return callRpc(this.serverConnection, 'resolve', [name, required]).then(function (resolutions) {
    ...
    return canConnect(resolution.host, resolution.port).then(function(direct) {
      if (direct) {
        debug('direct connecting to service');
        ...
        debug('proxying through server');

        return function () {
          var params = [].slice.call(arguments);
          if (resolution.type === 'callback') {
            var cb = params.pop();
            return callRpc(self.serverConnection, 'invoke', [resolution.id, params]).nodeify(cb);
          } else if (resolution.type === 'promised') {
            return callRpc(self.serverConnection, 'invoke', [resolution.id, params]);
          } else {
            throw new Error('Unsupported resolution type: %s', resolution.type);
          }
        };
