...
ServicifyServer.prototype.listen = function (opts) {
  ...
  var methods = {
    ...
    invoke: function(serviceId, params) {
      ...
      if (!serviceConn) return Promise.reject(new Error('Service not found: ' + serviceId));
      ...
      return new Promise(function(resolve) {
        function handleResponse(response) {
          ...
          if (msg.id === requestId && msg.result) {
            debug('response received for %s: %j', requestId, msg.result);
        ...
        debug('sending invoke request %s to %s with params %j', requestId, serviceId, params);
  ...
  sockjsJsonRpc.on('connection', function(conn) {
    conn.on('data', function(invocations) {
      jsonRpcProcessor(JSON.parse(invocations), methods, {conn: conn}).then(function(result) {
        if (result !== undefined) {
