...
ServicifyServicer.prototype.offer = function (target, spec) {
  ...
  return port.then(function (port) {
    ...
    var serverIo = require('socket.io-client')('http://' + self.opts.host + ':' + self.opts.port, {
      path: '/servicify',
      transports: ['websocket']
    });

    serverIo.on('error', function (err) {
      console.error('serverIO error', err);
    });

    serverIo.on('jsonrpc', function (invocations) {
      if (invocations) {
        jsonRpcProcessor(invocations, jsonRpcMethods).then(function (result) {
          serverIo.emit('jsonrpc', result);
        });
      }
    });
    ...
    function sendOffer(offering) {
      ...
      return callServerRpc('offer', [offering]);
    }

    function callServerRpc(method, params) {
      if (serverIo) {
        return new Promise(function (resolve, reject) {
          var requestId = uniqid();

          function handleResponse(response) {
            console.log('handle server rpc response', response);
            if (response && response.id === requestId) {
              serverIo.removeListener(handleResponse);
              if (response.error) {
          ...
          serverIo.on('jsonrpc', handleResponse);
          ...
          debug('sending request to server rpc %j', request);
          ;
          serverIo.emit('jsonrpc', request);
        });
      } else {
        return callRpc(self.serverConnection, method, params);
      }
    }
  });
};
